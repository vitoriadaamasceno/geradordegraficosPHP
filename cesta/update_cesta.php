...
$stmt = $conn->prepare("SELECT * FROM cestamaria WHERE idmaria = :ID");
...
<form action="update-action_cesta.php" method="POST">
  <input type="hidden" name=id_maria value="<?= $item['idmaria'];?>" readonly>
  <div class="mb-3 container">
    <label for="ano_maria" class="form-label">Ano:</label>
    <input type="text" class="form-control" name="ano_maria" id="ano_maria" value="<?= $item['anomaria'];?>">
  </div>
  <div class="mb-3 container">
    <label for="valor_maria" class="form-label">Valor</label>
    <input type="text" class="form-control" name="valor_maria" id="valor_maria" value="<?= $item['valormaria'];?>">
  </div>
  <div class="container">
